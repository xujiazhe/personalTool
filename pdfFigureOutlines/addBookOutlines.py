...
def addOutline(pdfw, outline_dict):
    ...
    olCnt = len(outline_dict)
    # get length of @pdf's _objects list; from this
    # we can derive the next and subsequent idorefs
    obCnt = len(pdfw._objects)
    idorefs = [PDF.IndirectObject(num + obCnt, 0, pdfw)
               for num in range(1, olCnt + 1)]
    ...
    ol.update({name("Type"): name("Outlines"),
               ...
               name("Last"): idorefs[-1],
               name("Count"): PDF.NumberObject(olCnt)})
    ...
    pdfw._root_object.getObject().update({name("Outlines"): idorefs[0]})
...
def main():
    ...
    # Value I/O error
    arg1, arg2 = os.path.join(bookDir, "figureBook.pdf"), "toc.json"
    ...
    oldict = json.load(g)
    addOutline(w, oldict)
    outputFile = open("bookWithOutlines.pdf", "wb")
    w.write(outputFile)
    outputFile.close()
    g.close()
    f.close()
